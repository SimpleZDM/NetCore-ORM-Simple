    ...
    public class SimpleMysqlTest
    {
        ...
        public void QueryTest()
        {
            try
            {
                ...
                //返回匿名对象
                var data0 = client.Queryable<UserEntity>().Select(u => new { Name = u.Name, Id = u.Idddd }).ToList();
                var data = client.Queryable<UserEntity>().Select(u => new UserEntity { Name = u.Name, Idddd = u.Idddd }).
                    Select(u => new { Name = u.Name, Id = u.Idddd }).ToList();
                //加条件
                int min = 1746;
                int max = 19999;
                var data1 = client.Queryable<UserEntity>().Where(user => user.Idddd > min && user.Idddd <= max).ToList();
                //分组
                var group = client.Queryable<UserEntity>().
                    Where(user => user.Idddd > min && user.Idddd <= max).
                    GroupBy(u => new { u.CompanyId }).Where(u=>u.Idddd > 100).ToList();
                //排序
                var order = client.Queryable<UserEntity>().Where(user => user.Idddd > min && user.Idddd <= max).OrderBy(u => new { u.Idddd }).ToList();
                var orderDesce = client.Queryable<UserEntity>().Where(user => user.Idddd > min && user.Idddd <= max).OrderByDescending(u => new { u.Idddd }).ToList();
                ...
                var JoinData1 = client.Queryable<
                   ...
                   Where((u, r, c) => u.Idddd > 10).OrderByDescending((u, r) => u.Idddd).
                   Select((u, r, c) => new
                   {
                       UserName = u.Name,
                ...
                var JoinData2 = client.Queryable<UserEntity, RoleEntity, CompanyEntity>((u, r, c) => new JoinInfoEntity(
                  ...
                  Where((u, r, c) => u.Idddd > 10).Select((u,r,c)=>
                  new ViewEntity {
                      UserName=u.Name,
                      CompanyId=c.Id,
                      CompanyName=c.CompanyName,
                ...
                var orderBy= client.Queryable<UserEntity, RoleEntity, CompanyEntity>((u, r, c) => new JoinInfoEntity(
                   ...
                   Where((u, r, c) => u.Idddd > 10).
                   Select((u, r, c) => new { UserName = u.Name, CompanyName = c.CompanyName, RoleName = r.DisplayName,Id=u.Idddd }).OrderBy(u=>u.Id).ToList();
                //Console.WriteLine($"*****************受影响行数:{result1}****************");
                // Console.WriteLine($"*****************受影响行数:{result2}****************");
                Console.WriteLine("****************测试结束*****************");
            }
