...
namespace NetCore.ORM.Simple.Visitor
{
    internal class SimpleVisitor
    {
        ...
        public SimpleVisitor(params Type[] types)
        {
            ...
            Select = new SelectEntity(types);
            mapVisitor = new MapVisitor(Select);
            joinVisitor = new JoinVisitor(Select.JoinInfos,Select.Table);
            OrderVistor = new OrderByVisitor(Select);
            conditionVisitor = new ConditionVisitor(Select.TreeConditions,Select.Conditions,Select.Table);
            conditionVisitor.InitMethodVisitor();
        }

        public List<JoinTableEntity> GetJoinInfos()
        {
            return Select.JoinInfos.Values.ToList();
        }
        public List<MapEntity> GetMapInfos()
        {
            return Select.MapInfos;
        }

        public Tuple<List<ConditionEntity>,List<TreeConditionEntity>> GetCondition()
        {
            return Tuple.Create(Select.Conditions,Select.TreeConditions);
        }
        ...
        public void VisitorCondition<T1>(Expression<Func<T1, bool>> expression)
        {
            ...
            conditionVisitor.Modify(expression,Select.MapInfos.ToArray());
        }
